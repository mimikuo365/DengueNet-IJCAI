...
def plotPredictions(city_ls):
    ...
    for city in city_ls:
      ...
      plotSelected(model_ls, swap_ls, selected_df, ground_true, root_folder / city, name)
...
def plotSelected(model_ls, swap_ls, df, ground_true, folder, city_name):
    ...
    # color_ls = ["tab:blue", "tab:orange", "tab:green", "tab:olive", "tab:purple", "tab:brown", 'tab:cyan']
    name = "_".join(model_ls)
    plt.figure(figsize=(8, 5))
    length = range(len(ground_true))
    plt.rcParams.update({'font.size': 14})
    plt.plot(length, ground_true, label="Ground Truth", color="tab:red")
    plt.plot([156-5-16, 156-5-16], [min(ground_true), max(ground_true)], linestyle='--', color="tab:gray")
    plt.plot([156-5-32, 156-5-32], [min(ground_true), max(ground_true)], linestyle='--', color="tab:gray")
    ...
    for i, row in df.iterrows():
      ...
      if (label == "Case" and "Case" in model_ls) or (label in model_ls and swap in swap_ls):
        text = getMap(label)
        if swap:
          text += ' (w/ CSR)'
    ...
    for city, leaky_rate in zip(city_ls, leaky_ls):
      ...
      for model_name in getModels(only_vit=True).keys():
        for swap in [True, False]:
          ...
          setting = getSetting(args.project_folder, swap, model_name, city, include_top=False, threshold_dic=None, 
                                selected_feature_ls=selected_feature_ls, threshold_cloud=cloud, threshold_shadow=shadow, 
                                feature_folder_name=feature_folder_name, lstm_week=lstm_week)
          
          model_parameter = DengueNet.getHyperparameter(leaky_rate=leaky_rate, vit_trainable=True) + "_" + str(len_callback)
          ...
          history = model.fit(data['train']['input'], data['train']['output'], epochs=10, batch_size=8,
          # history = model.fit(data['train']['input'], data['train']['output'], epochs=100, batch_size=8,
                              validation_data=(data['val']['input'], data['val']['output']), verbose=0,
          ...
          # print(compared_data.shape, all_heatmap_data.shape)
