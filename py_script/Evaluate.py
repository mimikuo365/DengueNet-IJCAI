              ...
              for vit_trainable in [True]: # Use trainable ViT model
                for lstm_week in lstm_week_ls: 
                  ...
                  for leaky_rate in leaky_rate_ls:
                    ...
                    df_location = Path(setting["result_folder"]) / selected_model / "result.csv"
                    # selected_model =  str(lstm_week) + f"_{normalize_vit}" + "/" + setting['model']+ "_" + str(swap)
                    model_parameter = DengueNet.getHyperparameter(leaky_rate=leaky_rate, vit_trainable=vit_trainable) + "_" + str(len_callback)
                    ...
                    history = model.fit(data['train']['input'], data['train']['output'], epochs=150, batch_size=8,
                    ...
                    for split in performance.keys():
                      for metric in performance[split].keys():
                        print(f"{split}_{metric}", performance[split][metric])
...
# from DataPreprocess import *
# from CalculateMetric import evalModel, plotModelStructure
# from DengueNet import DengueNet
# from ModelUtil import createTuner, getCallback, getModels, getResultLocation, getInput
...
#       for model_name in model_dic.keys():
#         swap_bool_ls = [False]
#         if model_dic[model_name] in [2, 5, 7, 8, 9]:
#           swap_bool_ls = [True, False]
        
#         for swap in swap_bool_ls:
#           for include_top in include_top_ls:
#             for cloud, shadow in threshold_pair_ls:
#                 print(model_name, swap, include_top, cloud, shadow)
#                 feature_folder_name = getFeatureFolderName(all_feature_ls, selected_feature_ls)
#                 setting = getSetting(args.project_folder, swap, model_name, city, 
...
#                 selected_model = setting['model']+ "_" + str(swap)
#                 model_parameter = DengueNet.getHyperparameter()
#                 model_location = getResultLocation(args, setting["model_folder"], selected_model, model_parameter + "/h5_model.h5")
#                 result_location = getResultLocation(args, setting["result_folder"], selected_model, model_parameter)
#                 print(result_location)
                
#                 if not os.path.exists(model_location):
#                   print(f"Error!! Folder {model_location} not existed!!")
#                   continue

#                 elif os.path.exists(model_location) and os.path.exists(result_location / "history.png"):
#                   print(f"Error!! Folder {model_location} already evaluated!!")
#                   continue
...
#                 if args.fixed_hyper:
#                   print("Used fixed hyper-parameter")
#                   model = keras.models.load_model(model_location)
#                 else:
#                   print("Used keras tuner model")
...
#         tf.config.set_logical_device_configuration(
#             gpus[0],
#             [tf.config.LogicalDeviceConfiguration(memory_limit=1024*args.gpu_size)])
