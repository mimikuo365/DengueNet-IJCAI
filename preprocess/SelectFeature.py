...
def evalSelectedFeature(
    ...
    result_location = Path(
        f"/mnt/usb/jupyter-mimikuo/feature_{learning_rate}_{swap}/callback{callback}/Normalize_{normalize_feature}/{city}/{selected_feature_ls}"
    )
    ...
    setting, data, labels, scaler = setup(
        setting, city, normalize_feature=normalize_feature
    )
    ...
    model_parameter = DengueNet.getHyperparameter(
        leaky_rate=0.2, vit_trainable=False, learning_rate=learning_rate
    )
    ...
    performance = evalModel(
        model, data, f, result_location, scaler, history, len_callback
    )
...
def main(args):
    ...
    for city in city_ls:
        best_feature_ls = []
        min_performance = {"feature": "", "mae": 10000}
        found_better = True
        ...
        while found_better:
            found_better = False
            second_min_performance = {"feature": "", "mae": 10000}
            for feature in all_feature_ls:
                ...
                performance = evalSelectedFeature(
                    args,
                    model_name,
                    city,
                    threshold_dic,
