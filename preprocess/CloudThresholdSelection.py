...
from DataPreprocess import splitDataset, getCityList, getCodeToNameMap, getSetting, setup, checkFolder
    ...
    for city in getCityList():
        ...
        for name, img in zip(name_ls, img_ls):
            if Path(f"{folder}/{name}.png").exists():
              continue
            fig, axs = plt.subplots(2, 10, figsize=(16, 5), dpi=300,
        ...
        for name, img in zip(name_ls, img_ls):
            ...
            for i, ax in zip(threshold_dic.keys(), fig.get_axes()):
                if i == 0:
                    ...
                    opening = cv.morphologyEx(np.float32(masked_img), cv.MORPH_OPEN, kernel)
                    print(city, name, i, ":", threshold_dic[i])
