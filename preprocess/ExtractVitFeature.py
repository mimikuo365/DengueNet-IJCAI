...
def readImage(path, normalized_img=False, target_size=(224, 224, 3)):
    ...
    image_test = skimage.transform.resize(
        image_test,
...
def getEmbeddings(path, use_np=True, normalized_img=False):
    ...
    for i, row in df.iterrows():
        ...
        feature_df = feature_df.append(
            {"embedding": embedding, "epiweek": epiweek}, ignore_index=True
        )
...
def scaleEmbeddings(path):
    ...
    for feature_idx in range(np_vit.shape[1]):
        ...
        for start, end in zip(
            [0, val_start, test_start], [val_start, test_start, size]
        ):
            print(np_vit[start:end, feature_idx].shape)
            np_vit[start:end, feature_idx] = scaler.transform(
                np_vit[start:end, feature_idx].reshape(-1, 1)
            ).flatten()
