...
def setParser(select_city=False):
    ...
    parser.add_argument(
        "--reverse_city_sequence", action=argparse.BooleanOptionalAction
    )
...
def setup(
    ...
    partition = splitDataset(
        df, "epiweek", num_train, num_train + num_val, lstm_week=setting["lstm_weeks"]
    )
    labels = splitDataset(
        df, "Cases", num_train, num_train + num_val, lstm_week=setting["lstm_weeks"]
    )
    metadata = splitDataset(
        pd.read_csv(setting["metadata_file"]),
        "temp",
        num_train,
        num_train + num_val,
    ...
    data, labels, scaler, setting = prepareData(
        data,
        setting,
        labels,
        metadata,
    ...
    if is_debug:
        for key in data.keys():
            print(
                key,
                data[key]["x_feng"].shape,
                data[key]["x_cnn"].shape,
                data[key]["case"].shape,
                labels[key].shape,
            )
...
def getInputDataset(
    ...
    if name == "train":
        ...
        start, end = int(total_len * train_ratio) - lstm_week, int(
            total_len * val_ratio
        )
    ...
    if type(setting["col"]) == type(None):
        with open(
            "/mnt/usb/jupyter-mimikuo/dataset/sorted/50001/feature/201601.pickle", "rb"
        ) as handle:
    ...
    for i in range(len(partition[name])):
        for band in [1, 2, 3, 11]:
            if band == 11:
                len_tile = setting[f"len_tile_{band}"] = int(
                    band_img_ls[0].shape[0] / setting["num_tile"]
                )
                img = band_img_ls[
                    i,
                    : setting["num_tile"] * len_tile,
                    : setting["num_tile"] * len_tile,
                ]
            else:
                setting[f"len_tile_{band}"] = int(
                    data[name]["reshape_img"][i].shape[0] / setting["num_tile"]
                )
                img = data[name]["reshape_img"][i][:, :, band - 1]

            label_ls, data[name][f"average_img_dic_{band}"] = swapLabelling(
                img,
                setting,
                data[name][f"average_img_dic_{band}"],
                setting[f"threshold_cloud_{band}"],
                setting[f"threshold_shadow_{band}"],
...
def prepareData(
    ...
    def getRadiomics(
        ...
        def getAvgFeature(data, setting):
            ...
            avg_feature = calculateSampleFeatures(
                avg_img, setting, getRadiomicSetting()
            )
        ...
        for split in splits:
            ...
            if setting["swap_enable"]:
                sample_feature = performTileSwapping(
                    sample_feature, dirty_label, avg_feature
                )

                if save_img:
                    img_end_idx = img_start_idx + dirty_label.shape[0]
                    origin_img = band_np[img_start_idx:img_end_idx, :, :].copy()
                    swapped_img = swapImgTile(
                        origin_img.copy(),
                        dirty_label,
                        avg_img,
                        setting["len_tile_11"],
                    ...
                    result_folder = Path(
                        f"/mnt/usb/jupyter-mimikuo/demo/{city}/swap_gray_resized/{setting['threshold_cloud']}_{setting['threshold_shadow']}"
                    )
                    checkFolder(result_folder / "comparison")
                    counter, start = plotSwapResultsGray(
                        origin_img, swapped_img, counter, start, result_folder, avg_img
                    )
                    img_start_idx = img_end_idx

            sample_feature = selectFeature(
                sample_feature, setting["col"][:-3], setting["selected_feature_ls"]
            )
        ...
        if normalize_feature:
            for feature_idx in range(sample_feature.shape[2]):
                ...
                for split in splits:
                    ...
                    data[split]["x_feng"][:, :, feature_idx] = scaler.transform(
                        tmp_data
                    )
    ...
    def getVit(
        ...
        counter = 1
        for split in splits:
            origin_img = data[split][
                "reshape_img"
            ].copy()  # Before time series: (124, 224, 224, 3)
            sample_img = data[split][
                "reshape_img"
            ].copy()  # Before time series: (124, 224, 224, 3)
            ...
            if setting["swap_enable"]:
                ...
                if save_img:
                    result_folder = Path(
                        f"/mnt/usb/jupyter-mimikuo/demo/{city}/swap_rgb_resized/{setting['threshold_cloud']}_{setting['threshold_shadow']}"
                    )
                    avg_img_rgb = np.array(avg_img_rgb)
                    checkFolder(result_folder / "single")
                    checkFolder(result_folder / "comparison")
                    counter, start = plotSwapResults(
                        data[split]["reshape_img"],
                        sample_img,
                        counter,
                        start,
                        result_folder,
                        np.transpose(avg_img_rgb, (1, 2, 0)),
                    )
