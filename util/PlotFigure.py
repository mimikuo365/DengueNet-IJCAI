...
def plotComparison(swapped_img, origin_img, result_folder, start, counter):
    # print("Is the two array the same??!: ", np.array_equal(swapped_img, origin_img))
...
def plotSwapResults(origin_img_ls, sample_img_ls, counter, start, result_folder, avg_img):
    avg_img = np.uint8((avg_img - tf.math.reduce_min(avg_img)) * 255 / (tf.math.reduce_max(avg_img) - tf.math.reduce_min(avg_img)))
    # print(max(avg_img.flatten()), min(avg_img.flatten()))
    # avg_img = keras.preprocessing.image.array_to_img(avg_img)
    # avg_img.save(result_folder / "average.png")
    ...
    for i in range(sample_img_ls.shape[0]):
        ...
        print(i) #, max(origin_img.flatten()), max(sample_img.flatten()), min(origin_img.flatten()), min(sample_img.flatten()))
        
        origin_img = np.uint8((origin_img - tf.math.reduce_min(origin_img)) * 255 / (tf.math.reduce_max(origin_img) - tf.math.reduce_min(origin_img)))
        sample_img = np.uint8((sample_img - tf.math.reduce_min(sample_img)) * 255 / (tf.math.reduce_max(sample_img) - tf.math.reduce_min(sample_img)))

        # print(i, max(origin_img.flatten()), max(sample_img.flatten()), min(origin_img.flatten()), min(sample_img.flatten()))
        plotSwappedImg(sample_img, result_folder / "single", start, counter)
        plotComparison(sample_img, origin_img, result_folder / "comparison", start, counter)
...
def plotSwapResultsGray(origin_img_ls, sample_img_ls, counter, start, result_folder, avg_img):
    # avg_img = np.uint8((avg_img - tf.math.reduce_min(avg_img)) * 255 / (tf.math.reduce_max(avg_img) - tf.math.reduce_min(avg_img)))
    # print(max(avg_img.flatten()), min(avg_img.flatten()))
    ...
    for i in range(sample_img_ls.shape[0]):
        ...
        plotComparisonGray(sample_img, origin_img, result_folder / "comparison", start, counter)
