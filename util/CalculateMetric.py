...
def plotHeatmap(data, scaled_pred, result_location, alpha=0.1):
  ...
  # for i, origin_img in enumerate(data['test']['input']):
      ...
      # print(origin_img[:1])
      # print((jet_heatmap * alpha)[:5])
      # print((jet_heatmap * alpha+origin_img)[:5])
      
      superimposed_img = jet_heatmap * alpha + origin_img
      superimposed_img = keras.preprocessing.image.array_to_img(superimposed_img)
      keras.preprocessing.image.array_to_img(origin_img).save(result_location / f"{i}_jet_norm.png")
...
def symmetricMeanAbsolutePercentageError(actual, predicted) -> float:
  ...
  return round(100 * np.mean((2 * np.abs(predicted - actual)) / (np.abs(actual) + np.abs(predicted))), 3)
...
def evalModel(origin_model, data, f, result_path, scaler, history, len_callback, trials=3):
  ...
  for i in range(trials):
    ...
    scaled_pred_ls = []
    model = origin_model
    _ = model.fit(data['train']['input'], data['train']['output'], epochs=10, batch_size=8,
    ...
    for pred, total_ls in zip([normal_pred_ls, scaled_pred_ls], [total_normal_pred_ls, total_scaled_pred_ls]):
  ...
  performance = writeMetric(data, avg_total_normal_pred_ls, np.array(total_normal_pred_ls), f, y_true_label="origin_output")
  plotPrediction(data, avg_total_normal_pred_ls, result_path, y_true_label="origin_output")
  return performance
...
def writeMetric(data, avg_pred, all_pred, f, y_true_label="origin_output") -> None:
  ...
  for split in data.keys():
    ...
    for i in range(len(all_pred)):
      pred = all_pred[i]
      print(pred)
      print(length, data[split][y_true_label].shape, pred.shape, pred[start:start+length].shape)
      mae, smape, rmse = computeMetric(data[split][y_true_label], pred[start:start+length])
    ...
    for metric in metric_name:
      ...
      f.write(f"  {metric}:   {performance[split][metric]}±{performance[split][std_name]}\n")
      print(f"  {metric}:   {performance[split][metric]}±{performance[split][std_name]}\n")
  ...
  #   performance[split] = {}
  #   f.write(f"{split}================================\n")
  #   len = data[split][y_true_label].shape[0]
  #   print(len, data[split][y_true_label].shape, pred.shape, pred[start:start+len].shape)
  #   mae, smape, rmse = computeMetric(data[split][y_true_label], pred[start:start+len])
  #   for metric, val in zip(["MAE", "sMAPE", "RMSE"], [mae, smape, rmse]):
  #     f.write(f"  {metric}:   {val:.2f}\n")
  #     performance[split][metric] = val
  #     # performance[split][f"{metric}_std"] = 
    
  #   for name, val in zip(["Real", "Pred"], [data[split][y_true_label], pred[start:start+len]]):
  #     x = val.flatten()
  #     f.write(f"  {name}:\n")
  #     f.write(("".join(['%d '] * x.size) + '\n') % tuple(x))
  #   f.write("\n")
  #   start = start + len
...
def plotPrediction(data, y_pred, folder, y_true_label):
  ...
  for i, key in enumerate(data.keys()):
    true = data[key][y_true_label]
    plt.plot([pre+len(true), pre+len(true)], [min_y, max_y], label='_nolegend_', color="g")
    pre += len(true)
  
  for y, name, color in zip([y_true, y_pred], ['GroundTruth', 'Prediction'], ['b', 'r']): 
