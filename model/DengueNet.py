...
class DengueNet(keras_tuner.HyperModel):
    ...
    @staticmethod
    def getHyperparameter(leaky_rate=None, vit_trainable=None, learning_rate=None) -> str:
        # print(leaky_rate, vit_trainable)
        name = ""
        if leaky_rate:
    ...
    def getCnnModel(include_top, img_shape):
        model = tf.keras.applications.MobileNetV3Small(include_top=include_top, weights='imagenet', input_shape=img_shape)
        # model = tf.keras.applications.MobileNetV2(include_top=include_top, weights='imagenet', input_shape=img_shape)
    ...
    def createLstmLayers(layer):
        for i, unit in enumerate(DengueNet.unit_ls):
            layer = layers.LSTM(units=unit, dropout=0.1, return_sequences=(i!=len(DengueNet.unit_ls)-1))(layer)
        return layer
    ...
    def getVitModel(self):
        ...
        layer = layers.TimeDistributed(vit_model, input_shape = ((lstm_week,) + img_shape))(vit_input)
    ...
    def getMobileNetModel(self):
        ...
        layer = layers.TimeDistributed(cnn_model, input_shape = ((lstm_week,) + img_shape))(cnn_input)
    ...
    def getConcatModel(self, layer_ls, input_ls, learning_rate=None, use_leaky=True, leaky_rate=None):
        if not leaky_rate:
            leaky_rate = DengueNet.leaky
        layer = concatenate(layer_ls, name="concatenated_layer")
        if use_leaky:
            for unit in DengueNet.nn_unit_ls:
                layer = Dense(units=unit, activation=tf.keras.layers.LeakyReLU(alpha=leaky_rate))(layer)
            # layer = Dense(units=DengueNet.nn_unit, activation=tf.keras.layers.LeakyReLU(alpha=leaky_rate))(layer)
            # layer = Dense(1, activation=tf.keras.layers.LeakyReLU(alpha=leaky_rate), name="output_layer")(layer)
        else:
            for unit in DengueNet.nn_unit_ls:
                layer = Dense(units=unit, activation="relu")(layer)
            # layer = Dense(units=DengueNet.nn_unit, activation="relu")(layer)
            # layer = Dense(1, activation="relu", name="output_layer")(layer)
        ...
        # model.compile(loss='mean_absolute_error', optimizer=opt, metrics=["mean_absolute_error"], jit_compile=True)
        return model
    ...
    def build(self, hp):
        ...
        if model_idx == 1:
            ...
            return self.createDengueNet(params["learning_rate"])
        elif model_idx == 8:
    ...
    def createFengVit(self, learning_rate=None):
        ...
        model = self.getConcatModel(layer_ls, input_ls, learning_rate, use_leaky=False, leaky_rate=0.2)
        return model
    ...
    def createDengueNet(self, learning_rate=None):
        eng_layer, eng_input = self.getFengModel()
        cnn_layer, cnn_input = self.getMobileNetModel()
